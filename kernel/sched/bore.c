...
struct sched_bore_data init_task_bore_data;

int alloc_task_bore(struct task_struct *p) {
	if (unlikely(p == &init_task)) {
		p->se.bore = &init_task_bore_data;
		return 0;
	}
	p->se.bore = kzalloc(sizeof(struct sched_bore_data), GFP_KERNEL);
	return p->se.bore ? 0 : -ENOMEM;
}

void free_task_bore(struct task_struct *p) {
	if (p->se.bore && p->se.bore != &init_task_bore_data)
		kfree(p->se.bore);
	p->se.bore = NULL;
}
...
void update_burst_score(struct sched_entity *se) {
	...
	if (!((p->flags & PF_KTHREAD) && likely(sched_burst_exclude_kthreads)))
		burst_score = se->bore->burst_penalty >> 2;
	se->bore->burst_score = burst_score;
...
static inline void init_task_burst_cache_lock(struct task_struct *p) {
	spin_lock_init(&p->se.bore->child_burst.lock);
	spin_lock_init(&p->se.bore->group_burst.lock);
}
...
static void update_burst_cache(struct sched_burst_cache *bc,
	...
	bc->score = max(avg, p->se.bore->burst_penalty);
	bc->count = cnt;
	bc->timestamp = now;
}
...
static inline void update_child_burst_direct(struct task_struct *p, u64 now) {
	...
	update_burst_cache(&p->se.bore->child_burst, p, cnt, sum, now);
}
...
static inline void update_tg_burst(struct task_struct *p, u64 now) {
	...
	for_each_thread(p, task) {
		if (!task_is_bore_eligible(task)) continue;
		cnt++;
		sum += task->se.bore->burst_penalty;
	}

	update_burst_cache(&p->se.bore->group_burst, p, cnt, sum, now);
}

static inline u8 inherit_burst_tg(struct task_struct *p, u64 now) {
	struct task_struct *parent = rcu_dereference(p->group_leader);
	struct sched_burst_cache *bc = &parent->se.bore->group_burst;
	guard(spinlock)(&bc->lock);
	if (burst_cache_expired(bc, now))
...
void sched_clone_bore(struct task_struct *p,
	...
	revolve_burst_penalty(se);
	se->bore->burst_penalty = se->bore->prev_burst_penalty =
		max(se->bore->prev_burst_penalty, penalty);
	se->bore->child_burst.timestamp = 0;
	se->bore->group_burst.timestamp = 0;
}

void reset_task_bore(struct task_struct *p) {
	p->se.bore->burst_time = 0;
	p->se.bore->prev_burst_penalty = 0;
	p->se.bore->curr_burst_penalty = 0;
	p->se.bore->burst_penalty = 0;
	p->se.bore->burst_score = 0;
	memset(&p->se.bore->child_burst, 0, sizeof(struct sched_burst_cache));
	memset(&p->se.bore->group_burst, 0, sizeof(struct sched_burst_cache));
}
...
void __init sched_bore_init(void) {
	...
	init_task.se.bore = &init_task_bore_data;
