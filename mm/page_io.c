...
static bool kcompressd_store(struct folio *folio)
{
	...
	struct kcompressd_pgdat *kc = pgdat->kcompressd_ext;
	...
	if (!sysctl_kcompressd || unlikely(!kc) || unlikely(!kc->task))
		return false;
	...
	/* If the fifo is full, we must swap out the head
	 * folio to make space for the new folio.
	 */
	scoped_guard(spinlock_irqsave, &kc->fifo_lock)
		if (kfifo_len(&kc->fifo) >= sysctl_kcompressd * sizeof(folio) &&
				unlikely(!kfifo_out(&kc->fifo, &head, sizeof(folio))))
	...
	ret = kfifo_in(&kc->fifo, &folio, sizeof(folio));
	if (likely(ret))
		/* We successfully enqueued the folio. wake up kcompressd */
		wake_up_interruptible(&kc->wait);
	else
...
int kcompressd(void *p)
{
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible(kc->wait,
				!kfifo_is_empty(&kc->fifo));

		while (kfifo_out_locked(&kc->fifo,
				&folio, sizeof(folio), &kc->fifo_lock))
			do_swapout(folio);
	}
	return 0;
}
